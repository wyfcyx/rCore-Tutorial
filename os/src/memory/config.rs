...
use crate::board::config::{
    BOARD_MEMORY_END_ADDRESS,
    BOARD_KERNEL_HEAP_SIZE,
};
...
/// 可以访问的内存区域结束地址
pub const MEMORY_END_ADDRESS: PhysicalAddress = PhysicalAddress(BOARD_MEMORY_END_ADDRESS);
...
/// 操作系统动态分配内存所用的堆大小（8M）
pub const KERNEL_HEAP_SIZE: usize = BOARD_KERNEL_HEAP_SIZE;
